...
{
    //cprintf("Freeing page %x\n", page2pa(pp));
	pp->pp_link = page_free_list;
	page_free_list = pp;
    struct Page * copy = page_free_list;
}
                ...
                int perm)
{
    ...
    for(i = 0; i < size; i += PGSIZE) {
      uintptr_t address_check = current_va + i;
      if(address_check < current_va) {
        break;
      }
      pte_t * pte = pgdir_walk(pgdir, (void *) current_va+i, 1);
      * pte = (physaddr_t) (current_pa+i) | perm | PTE_P;
    }
}
...
page_insert(pde_t *pgdir, struct Page *pp, void *va, int perm)
{
    ...
    * pte_p = (physaddr_t) (page2pa(pp) | perm | PTE_P);
	return 0;
}
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	//
	// Your code here:
	size = size%PGSIZE == 0?size:size + (PGSIZE - size%PGSIZE);
	if ((base + size) >= MMIOLIM){
		panic("mmio_map_region tried to write past MMIOLIM.");
	}
	boot_map_region(kern_pgdir, base, size, pa, PTE_PCD|PTE_PWT|PTE_W);
	uintptr_t base_tmp = base;
	base += size;
	return (void*)base_tmp;
}
