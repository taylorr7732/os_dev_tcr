...
trap_init(void)
{
    ...
    SETGATE(idt[T_GPFLT], 0, GD_KT, &GpFault, 0);
    SETGATE(idt[T_PGFLT], 0, GD_KT, &PgFault, 0);
    SETGATE(idt[T_FPERR], 0, GD_KT, &FPError, 3);
    SETGATE(idt[T_ALIGN], 0, GD_KT, &Align, 0);
    SETGATE(idt[T_MCHK], 0, GD_KT, &MCheck, 0);
    SETGATE(idt[T_SIMDERR], 0, GD_KT, &SIMDError, 0);
...
page_fault_handler(struct Trapframe *tf)
{
    ...
    if(curenv->env_pgfault_upcall){
        uintptr_t i;
        uintptr_t* stack = (void*)(UXSTACKTOP) - 4;
        bool isRecursive = tf->tf_esp >=(UXSTACKTOP-PGSIZE) && 
                           tf->tf_esp <= (UXSTACKTOP-4);
                           
        if(isRecursive){
            stack = ((void*)tf->tf_esp)-8;
        }
        user_mem_assert(curenv, stack - sizeof(struct UTrapframe),
                                sizeof(struct UTrapframe), 0);

        *(stack) = tf->tf_esp;
        *(stack-1) = tf->tf_eflags;
        *(stack-2) = tf->tf_eip;
        *(stack-3) = tf->tf_regs.reg_eax;
        *(stack-4) = tf->tf_regs.reg_ecx;
        *(stack-5) = tf->tf_regs.reg_edx;
        *(stack-6) = tf->tf_regs.reg_ebx;
        *(stack-7) = tf->tf_regs.reg_oesp;
        *(stack-8) = tf->tf_regs.reg_ebp;
        *(stack-9) = tf->tf_regs.reg_esi;
        *(stack-10) = tf->tf_regs.reg_edi;
        *(stack-11) = tf->tf_err;
        *(stack-12) = fault_va;

       
        curenv->env_tf.tf_esp =  (uintptr_t)(stack-12);
        user_mem_assert(curenv, curenv->env_pgfault_upcall, PGSIZE, 0);
